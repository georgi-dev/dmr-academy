...
(function (factory) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	FC.getScrollbarWidths = _getScrollbarWidths;
 ...
 */
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		renderHelper: function renderHelper(eventLocation, sourceSeg) {
			// subclasses must implement
		},
  ...
  */
		...
		renderFill: function renderFill(type, segs) {
			// subclasses must implement
		},
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		// Must return elements used for any mock events.
		renderEventResize: function renderEventResize(range, seg) {
			// subclasses must implement
		},
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	function getDraggedElMeta(el) {
		...
		if (eventProps) {
			if ((typeof eventProps === 'undefined' ? 'undefined' : _typeof(eventProps)) === 'object') {
 ...
 */
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		renderHeadDateCellHtml: function renderHeadDateCellHtml(date, colspan, otherAttrs) {
			...
			return '' + '<th class="' + classNames.join(' ') + '"' + (this.rowCnt === 1 ? ' data-date="' + date.format('YYYY-MM-DD') + '"' : '') + (colspan > 1 ? ' colspan="' + colspan + '"' : '') + (otherAttrs ? ' ' + otherAttrs : '') + '>' +
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		renderNumberCellHtml: function renderNumberCellHtml(date) {
			...
			if (this.view.cellWeekNumbersVisible && date.day() == weekCalcFirstDoW) {
				html += this.view.buildGotoAnchorHtml({ date: date, type: 'week' }, { 'class': 'fc-week-number' }, date.format('w' // inner HTML
				));
			}

			if (this.view.dayNumbersVisible) {
				html += this.view.buildGotoAnchorHtml(date, { 'class': 'fc-day-number' }, date.date // inner HTML
				());
			}
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
		...
		rejectOn: function rejectOn(eventName, promise) {
			var _this = this;
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		handleDate: function handleDate(date, isReset) {
			var _this = this;
		...
		requestDateRender: function requestDateRender(date) {
			var _this = this;
		...
		// if date not specified, uses current
		executeDateRender: function executeDateRender(date) {
			var _this = this;
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		updateWidth: function updateWidth(isResize) {
			// subclasses should implement
		},
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		captureScroll: function captureScroll() {
		...
		captureInitialScroll: function captureInitialScroll(forcedScroll) {
		...
		hardSetScroll: function hardSetScroll(scroll) {
			var _this = this;
		...
		setScroll: function setScroll(scroll) {},
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		handleEvents: function handleEvents(events, isReset) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
					...
					$.each(this.split(','), function (j, buttonName) {
						...
						if (buttonName == 'title') {
							...
							if (buttonClick) {
								...
								themeIcon = customButtonProps ? customButtonProps.themeIcon : calendar.options.themeButtonIcons[buttonName];
	...
	function Calendar_constructor(element, overrides) {
		...
		function renderView(viewType, forcedScroll) {
			...
			if (currentView) {
				...
				date >= currentView.intervalStart && date < currentView.intervalEnd)) {
					if (elementVisible()) {
  ...
  -----------------------------------------------------------------------------*/
		...
		function buildToolbars() {
			return [new Toolbar(t, computeHeaderOptions()), new Toolbar(t, computeFooterOptions())];
		}
 ...
 */
  ...
  -----------------------------------------------------------------------------*/
		...
		function buildEventSource(sourceInput) {
			...
			} else if ((typeof sourceInput === 'undefined' ? 'undefined' : _typeof(sourceInput)) === 'object') {
		...
		function getSourcePrimitive(source) {
			return ((typeof source === 'undefined' ? 'undefined' : _typeof(source)) === 'object' ? // a normalized event source?
 ...
 -----------------------------------------------------------------------------------------*/
	...
	Calendar.prototype.constraintToEvents = function (constraintInput) {
		...
		if ((typeof constraintInput === 'undefined' ? 'undefined' : _typeof(constraintInput)) === 'object') {
